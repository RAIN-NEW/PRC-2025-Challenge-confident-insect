...
"""
Repair a single PRC-2025 trajectory parquet (Rank Dataset) and write to the output directory.
...
def parse_args():
    ap = argparse.ArgumentParser(description="Batch repair trajectory Parquet files (Rank Dataset).")
    
    # Dynamically locate project root
    # Assumes script is at: confident-insect/src/preprocessing/rank/01_complement_data.py
    ...
    # Adjusted for RANK dataset: flights_rank
    default_input = project_root / "data" / "raw" / "prc-2025-datasets" / "flights_rank"
    
    # Output path structure matches previous logic
    default_output = project_root / "data" / "intermediate" / "01_complement_data" / "flights_rank"
