...
"""
Generate Samples By Aircraft Type (Final Production Version).

Features:
- **Inference Mode**: Adapted for Final Inference phase (No fuel label dependencies).
...
# Assumes script is at: confident-insect/src/preprocessing/final/02_build_script.py
...
# Input: Previous step's output (trajectories) + Metadata (fuel_final_submission, flightlist_final)
DEFAULT_ROOT = project_root / "data" / "intermediate" / "01_complement_data"

# Output: Current step's output directory
DEFAULT_OUT = project_root / "data" / "intermediate" / "02_build_script" / "final"
...
def parse_args():
    ap = argparse.ArgumentParser(description="Generate final samples from trajectory data.")
    ...
    ap.add_argument("--root", type=str, default=str(DEFAULT_ROOT),
                    help=f"Input directory containing flights_final/ and metadata. Default: {DEFAULT_ROOT}")
...
def load_traj(fid, root_dir):
    """Load trajectory file for a given flight ID (from flights_final)."""
    f = root_dir / "flights_final" / f"{fid}.parquet"
    if not f.exists():
        return None
    try:
...
def process_single_row(row, id2type, root_dir):
    ...
    # [Final/Inference Update] Removed fuel_kg check
    ...
    # [Final/Inference Update] No fuel rate calculation
    ...
    stat = {
        ...
        # "fuel_kg": float(row.fuel_kg), # Removed for inference
...
def flush_buffer_single_file(ac_type, buffer_samples, out_dir, writers):
    ...
    # Enforce Schema ([Final/Inference Update] No fuel_rate_warm)
    target_cols = [
...
def main():
    ...
    # Load metadata ([Final Update] Use fuel_final_submission and flightlist_final)
    final_file = ROOT / "fuel_final_submission.parquet"
    list_file = ROOT / "flightlist_final.parquet"
    
    print(f"Reading final data: {final_file}")
    try:
        fuel_df = pd.read_parquet(final_file)
    ...
    # Safety check for submission index
    if "idx" not in fuel_df.columns:
        print("Warning: 'idx' column missing from input. This might cause errors downstream.")
    ...
    print(f"Processing {len(fuel_df)} final tasks with Stream Flushing...")
