...
"""
Generate Samples By Aircraft Type (Final Production Version) for Training Data.

Features:
- **Vectorized Logic**: Includes fixes for IDL crossing and 1-to-Many ID mappings.
...
- **Strict Validation**: Enforces the existence of the 'idx' column.
...
# Assumes script is at: confident-insect/src/preprocessing/train/02_build_script.py
...
# Input: Previous step's output (trajectories) + Metadata (fuel_train, flightlist)
# Note: Ensure 'fuel_train.parquet' and 'flightlist_train.parquet' are present in this directory.
DEFAULT_ROOT = project_root / "data" / "intermediate" / "01_complement_data"

# Output: Current step's output directory
DEFAULT_OUT = project_root / "data" / "intermediate" / "02_build_script" / "train"
...
def parse_args():
    ap = argparse.ArgumentParser(description="Generate training samples from trajectory data.")
    ...
    ap.add_argument("--root", type=str, default=str(DEFAULT_ROOT),
                    help=f"Input directory containing flights_train/ and metadata. Default: {DEFAULT_ROOT}")
...
def load_traj(fid, root_dir):
    """Load trajectory file for a given flight ID."""
    f = root_dir / "flights_train" / f"{fid}.parquet"
    if not f.exists():
        return None
    try:
...
def process_single_row(row, id2type, root_dir):
    ...
    # Calculate fuel rate
    actual_r = float(row.fuel_kg) / total_time if total_time > 1e-9 else 0.0
    need["fuel_rate_warm"] = np.float64(np.round(actual_r, 4))
    ...
    stat = {
        ...
        "fuel_kg": float(row.fuel_kg), "covered_time": covered_time,
        ...
        "ac_type": ac_type,
        "idx_orin": row["idx"], # Strict check for index alignment
...
def flush_buffer_single_file(ac_type, buffer_samples, out_dir, writers):
    ...
    target_cols = [
        ...
        "dt", "gs_vn", "gs_ve", 
        "interval_id", "fuel_rate_warm", "flight_id", "aircraft_type"
    ]
    ...
    float_cols = ["latitude", "longitude", "altitude", "groundspeed", "track", "vertical_rate", "gs_vn", "gs_ve", "fuel_rate_warm", "dt"]
    for c in float_cols:
...
def main():
    ...
    # Load metadata
    # Note: These files are expected to be in the input root directory
    fuel_train = pd.read_parquet(ROOT / "fuel_train.parquet")
    flightlist = pd.read_parquet(ROOT / "flightlist_train.parquet")[["flight_id", "aircraft_type"]]
    id2type = dict(zip(flightlist.flight_id.values, flightlist.aircraft_type.values))
    
    # Ensure datetime format
    for col in ["start", "end"]:
        if not np.issubdtype(fuel_train[col].dtype, np.datetime64):
            fuel_train[col] = pd.to_datetime(fuel_train[col], utc=True)
    
    print(f"Processing {len(fuel_train)} tasks with Stream Flushing...")
    ...
    try:
        with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
            futures = [
                ...
                for _, row in fuel_train.iterrows()
            ]
