...
"""
Generate ERA5 Weather Data Download List (Final Dataset).

Purpose:
    Scans trajectory statistics (interval_stats.parquet) from the Final dataset to determine 
    which ERA5 grid tiles, dates, hours, and pressure levels are required.
...
Input: interval_stats.parquet files from '02_build_script/final'
Output: JSON file containing download instructions (era5_download_final_GRID.json).
"""
...
# Assumes script is at: confident-insect/src/tools/generate_era5_jobs_final.py
...
# Define paths relative to project root (Final Dataset)
DEFAULT_ROOT = project_root / "data" / "intermediate" / "02_build_script" / "final"
DEFAULT_OUT = project_root / "data" / "intermediate" / "climate_datasets" / "download_lists" / "era5_download_final_GRID.json"
...
def parse_args():
    ap = argparse.ArgumentParser(description="Generate ERA5 Download List (Final Data)")
