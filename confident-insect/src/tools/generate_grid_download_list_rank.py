...
"""
Generate ERA5 Weather Data Download List (Rank Dataset).

Purpose:
    Scans trajectory statistics (interval_stats.parquet) from the Rank dataset to determine 
...
Input: interval_stats.parquet files from '02_build_script/rank'
Output: JSON file containing download instructions (era5_download_rank_GRID.json).
"""
...
# Assumes script is at: confident-insect/src/tools/generate_era5_jobs_rank.py
...
DEFAULT_ROOT = project_root / "data" / "intermediate" / "02_build_script" / "rank"
DEFAULT_OUT = project_root / "data" / "intermediate" / "climate_datasets" / "download_lists" / "era5_download_rank_GRID.json"
...
def parse_args():
    ap = argparse.ArgumentParser(description="Generate ERA5 Download List (Rank Data)")
